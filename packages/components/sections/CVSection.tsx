import { CV } from "@research-homepage/cms";
import { Card, CardContent, CardHeader, Badge, Button } from "@research-homepage/ui";
import { Download, FileText, Calendar, FileIcon } from "lucide-react";

interface CVSectionProps {
  cvs: CV[];
  /** Sanity project ID for constructing file URLs */
  projectId: string;
  /** Sanity dataset name for constructing file URLs */
  dataset: string;
}

export default function CVSection({ cvs, projectId, dataset }: CVSectionProps) {
  if (!cvs || cvs.length === 0) return null;

  const primaryCV = cvs.find(cv => cv.isPrimary) ?? cvs[0];
  if (!primaryCV) return null; // TypeScript guard - shouldn't reach due to length check above
